...
namespace EasySwoole\Whoops\Exception;
...
class Inspector
{
    ...
    public function getFrames()
    {
        if ($this->frames === null) {
            ...
            // If the parameter cannot be serialized, only the class name of the parameter is displayed
            foreach ($frames as $k => $frame) {
                ...
                if (!empty($frame['args'])) {
                    foreach ($frame['args'] as $index => $arg) {
                        // 资源和对象都转字符串
                        if (is_resource($arg)) {
                            $resourceName = var_export($arg, true);
                            $frames[$k]['args'][$index] = $resourceName;
                        }
                        // 闭包无法被序列化
                        if ($arg instanceof \Closure) {
                            $frames[$k]['args'][$index] = 'Anonymous Closure';
                        }
                        // 如果是对象则取对象名称
                        if (is_object($arg)) {
                            $className = get_class($arg);
                            $frames[$k]['args'][$index] = $className;
                        }
                        // 如果是数组则JSON转换
                        if (is_array($arg)) {
                            $frames[$k]['args'][$index] = json_decode(json_encode($arg), true);
                        }
                    }
                }
            }
    ...
    protected function getTrace($e)
    {
        ...
        // swoole does not support xdebug extensions so no inspection is required.
